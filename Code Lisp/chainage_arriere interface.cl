(defun beginArriere()
  ...
  (let (Taille_de_ecran Marque Performance Nouveau_modele BudgetMax Memoire choix target verifier)
   ...
   (format t "~%Ensuit, entrez votre demande pour votre téléphone, s'il vous plaît: ~%")
    ...
    (if (equal verifier nil)
        (format t "~%Le telephone choisi ne correspond pas à vos critères. Vous pouvez ré-essayer. ~%~%")
        (format t "~%Le telephone choisi bien correspond à vos critères! ~%~%")
      )
   )
  'Bye
  )
